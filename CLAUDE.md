...
2. **POMs must extend `BasePage`.** BasePage is the *only* place Playwright's `Locator`/`Page` APIs are consumed for actions and assertions. POMs define locators inline (e.g., `this.page.getByLabel(...)`, `this.page.getByRole(...)`) but every click/fill/assert goes through `BasePage` helpers (`clickOnElement`, `fillElementWithText`, `assertElementIsVisible`, `assertElementContainsText`, …). Each helper takes an `elementDescription` string for the log line.
   ...
   **Every POM must declare `protected readonly urlPath = strings.pages.<key>.urlPath`.** BasePage exposes the abstract field `protected abstract readonly urlPath: string` and a public `goto()` that uses it (`await this.navigateToUrlPath(this.urlPath)`). Tests reach a page via `pageManager.xxxInstance().goto()` — POMs do **not** define their own `navigateToXxxPage()` wrapper (that pattern was removed because it duplicated a one-line method across every POM). TypeScript will fail the build if a new POM forgets `urlPath`.
...
- **New page object:** add **only user-facing UI strings** under `pages.newPage.*` in `src/utils/strings.json` (accessible names, visible text, URL paths, app-rendered messages — see rule #1). **Do NOT add a `descriptions.*` sub-object** — locator `.describe(...)` text and `elementDescription` log text are internal and inlined as literal strings at the `.describe(...)` and BasePage helper call sites (rule #3a; do not centralize into a `DESCRIPTIONS` map). Create `src/pages/NewPage.ts` extending `BasePage`, `import strings from '../utils/strings.json'`, reference the strings via `strings.pages.newPage.*`, **declare `protected readonly urlPath = strings.pages.newPage.urlPath` near the top of the class** (so the inherited `goto()` works — see rule #2), and register the POM in `PageManager` (field + constructor line + `newPageInstance()` accessor). Do NOT add a per-POM `navigateToXxxPage()` method — `goto()` is inherited from BasePage. If a string has a `{placeholder}`, substitute it at the call site with `.replace('{placeholder}', value)`.
